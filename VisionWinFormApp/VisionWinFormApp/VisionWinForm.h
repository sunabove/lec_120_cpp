...
private: System::Void FileSaveCsv(System::String^ fileName ) { 
			 ...
			 for( size_t i = 0 , iLen = dataCnt ; i < iLen ; i ++ ) { 
				 ...
				 if( i < fxSize ) { 
					 ...
					 sprintf_s( fxText, "  ,   ,   ,     " , freq, amp, real, imag );
					 sprintf_s( fyText, "  ,   ,   ,     " , freq, amp, real, imag );
				 }
...
/*
private: System::Void FileSaveXls(System::String^ fileName ) {  

			 // excel file save 

			 if( true ) { 
				 // show message to save the excel file
				 System::String^ title = L"Warning";
				 System::String^ msg = L"엑셀 챠트 저장 기능에는 버그가 있을 수 있습니다. 그래도 계속하시겠습니까?";

				 if( System::Windows::Forms::DialogResult::No == System::Windows::Forms::MessageBox::Show( msg, title, MessageBoxButtons::YesNo, MessageBoxIcon::Exclamation ) ) { 
					 return ; 
				 } 
			 }
			 ...
			 if( nullptr != fileName ) {   
				wb = exApp->Workbooks->Add(Type::Missing);
			 } else {
				wb = exApp->Workbooks->Add(Type::Missing);
			 }
			 ...
			 if( false ) { 
				 safe_cast<Excel::Worksheet^>(exApp->ActiveWorkbook->Sheets[3])->Delete();
				 safe_cast<Excel::Worksheet^>(exApp->ActiveWorkbook->Sheets[2])->Delete();
			 }

			 //4. Create a variable for the active Work sheet's tracking handle
			 //  (first Worksheet is the default active one)
			 Excel::Worksheet^ ws = safe_cast<Excel::Worksheet^>(exApp->ActiveSheet);
			 ...
			 const size_t spectrumCnt = xSpec.size() ;
			 const double fps = this->visionChartFreq->GetFps() ;
			 const double maxFreq = fps ;
			 const double unitFreq = maxFreq/spectrumCnt ; 
			 ...
			 const size_t dxCnt = dxList->Count ;
			 const size_t dyCnt = dyList->Count ;
			 const size_t fxCnt = xSpec.size();
			 const size_t fyCnt = ySpec.size(); 
			 ...
			 if( true ) {
				 ...
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = L"Date:";
				 ws->Cells[ row, col ++ ] = L"" + now->Year + "-" + now->Month + "-" + now->Day  ; 
				 ws->Cells[ row, col ++ ] = L"" + now->Hour + ":" + now->Minute + ":" + now->Second ; 
			 }
			 ...
			 if( true ) {
				 ...
				 ws->Cells[ row, col ] = L"변환 작업중입니다. 작업 종료시까지 클릭하지 마세요!" ; 

				 row ++;
				 statRow = row; 
				 col = 5 ; 
				 statCol = 6; 
				 ws->Cells[ row, col ++ ] = "변환 중 : " ; 
				 ws->Cells[ row, col ++ ] = "0%" ;
			 ...
			 if( true ) {
				 ...
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, 4 ] = L"dX Count:" ;
				 ws->Cells[ row, 5 ] = L"" + dxCnt ; 

				 row ++ ; 
				 ws->Cells[ row, 4 ] = L"dY Count:" ;
				 ws->Cells[ row, 5 ] = L"" + dyCnt ; 

				 row++ ;
				 ws->Cells[ row, 4 ] = L"fX Count:" ;
				 ws->Cells[ row, 5 ] = L"" + fxCnt ; 

				 row++ ; 
				 ws->Cells[ row, 4 ] = L"fY Count:" ;
				 ws->Cells[ row, 5 ] = L"" + fyCnt ; 
			 }
			 ...
			 if( true ) {  
				 row ++ ; 
				 col = 1;
				 //sprintf_s( text, "%d, , %f, %f, %f, , %s, , %s", (i + 1), timeSec, (double) dxList[i], (double) dyList[i], fxText, fyText );
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "NO" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "Time(sec)" ; 
				 ws->Cells[ row, col ++ ] = "DX(mm)" ; 
				 ws->Cells[ row, col ++ ] = "DY(mm)" ; 
			 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "No" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "FREQ_X(Hz)" ; 
				 ws->Cells[ row, col ++ ] = "AMP_X" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "REAL_X" ; 
				 ws->Cells[ row, col ++ ] = "IMAG_X" ; 
			 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "No" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "FREQ_Y(Hz)" ; 
				 ws->Cells[ row, col ++ ] = "AMP_Y" ; 
				 ws->Cells[ row, col ++ ] = "" ; 
				 ws->Cells[ row, col ++ ] = "REAL_Y" ; 
				 ws->Cells[ row, col ++ ] = "IMAG_Y" ;  

				 ws->Range[ "A1", Type::Missing]->EntireColumn->ColumnWidth = 2; 
				 ws->Range[ "B1", Type::Missing]->EntireColumn->ColumnWidth = 5; 
				 ws->Range[ "C1", Type::Missing]->EntireColumn->ColumnWidth = 2; 
				 ws->Range[ "D1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ "E1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ "F1", Type::Missing]->EntireColumn->ColumnWidth = 10; 

				 ws->Range[ L"G1", Type::Missing]->EntireColumn->ColumnWidth = 2; 
				 ws->Range[ L"H1", Type::Missing]->EntireColumn->ColumnWidth = 5; 
				 ws->Range[ L"I1", Type::Missing]->EntireColumn->ColumnWidth = 2; 
				 ws->Range[ L"J1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ L"K1", Type::Missing]->EntireColumn->ColumnWidth = 10;
				 ws->Range[ L"L1", Type::Missing]->EntireColumn->ColumnWidth = 2;
				 ws->Range[ L"M1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ L"N1", Type::Missing]->EntireColumn->ColumnWidth = 10; 

				 ws->Range[ L"O1", Type::Missing]->EntireColumn->ColumnWidth = 2; 
				 ws->Range[ L"P1", Type::Missing]->EntireColumn->ColumnWidth = 5; 
				 ws->Range[ L"Q1", Type::Missing]->EntireColumn->ColumnWidth = 2; 
				 ws->Range[ L"R1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ L"S1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ L"T1", Type::Missing]->EntireColumn->ColumnWidth = 2;
				 ws->Range[ L"U1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
				 ws->Range[ L"V1", Type::Missing]->EntireColumn->ColumnWidth = 10; 
			 }

			 int rowNo ;

			 int dataCnt = (int) ( dxCnt > fxCnt ? dxCnt :fxCnt ) ; 

			 int convCnt = 0;
			 int totCnt = (int) ( dxCnt*2 + fxCnt*2 ) ; 
			 
			 char text[ 100 ];

			 for( int i = 0 , iLen = dataCnt ; i < iLen ; i ++ ) {
				 // add cells
				 ...
				 if( i < dxCnt ) { 
					 ...
					 ws->Cells[ row, col ++ ] = "" ; 
					 ws->Cells[ row, col ++ ] = rowNo ; 
					 ws->Cells[ row, col ++ ] = "" ; 
					 ws->Cells[ row, col ++ ] = timeSec ;   
					 ws->Cells[ row, col ++ ] = dxList[i] ; 
					 ws->Cells[ row, col ++ ] = dyList[i] ; 
				 ...
				 if( i < fxCnt ) { 
					 ...
					 //sprintf_s( fxText, "%f, %f, %f, i*%f" , freq, amp, real, imag );

					 ws->Cells[ row, col ++ ] = "" ; 
					 ws->Cells[ row, col ++ ] = rowNo ; 
					 ws->Cells[ row, col ++ ] = "" ; 
					 ws->Cells[ row, col ++ ] = freq ; 
					 ws->Cells[ row, col ++ ] = amp ; 
					 ws->Cells[ row, col ++ ] = "" ; 
					 ws->Cells[ row, col ++ ] = real ; 
					 ws->Cells[ row, col ++ ] = imag ; 
				 ...
				 double pct = convCnt/(totCnt + 0.0)*100 ; 
				 sprintf_s( text, "%.1f%s", pct, "%" );

				 ws->Cells[ statRow, statCol ] = gcnew System::String( text ) ;
				 // end of adding cells
			 }

			 ws->Cells[ msgRow, msgCol ] = L"데이터 변환 완료." ;  

			 int		xPos = 20*48;      //Col width 48 points. Chart starts in 3rd col
			 int		yPos = 7*9;           //Row height = 9, Chart starts in 2nd row 
			 int		width = 350;
			 int		height = 300 ; 

			 int		chartRangeRow = 12; 
			 if( true ) {
				 // add chart 
				 System::String^ range = L"D" + chartRangeRow ;  

				 System::String^ title = "Displacement"; 
				 System::String^ xAxisTitle = "time (sec)" ;
				 System::String^ yAxisTitle = "Disp(mm)";

				 bool b  = AddChart( ws, range, xPos, yPos, width, height, title, xAxisTitle, yAxisTitle ) ;   
				 ws->Cells[ msgRow, msgCol ] = b ? L"변위 챠트 생성 완료." : L"변위 챠트 생성 실패" ;  
			 }

			 if( true ) {
				 // add frequency x chart
				 yPos += ( 19*9 );

				 System::String^ range = L"J" + chartRangeRow ;  

				 System::String^ title = "Frequency X"; 
				 System::String^ xAxisTitle = "Frequency (Hz)" ;
				 System::String^ yAxisTitle = "Amplitude";

				 bool b = AddChart( ws, range, xPos, yPos, width, height, title, xAxisTitle, yAxisTitle ) ;  
				 ws->Cells[ msgRow, msgCol ] = b ? L"주파수(X) 챠트 생성 완료." : L"주파수(X) 챠트 생성 실패." ;  
			 }

			 if( true ) {
				 // add frequency y chart
				 yPos += ( 19*9 );

				 System::String^ range = L"R" + chartRangeRow ;  

				 System::String^ title = "Frequency Y"; 
				 System::String^ xAxisTitle = "Frequency (Hz)" ;
				 System::String^ yAxisTitle = "Amplitude";

				 bool b = AddChart( ws, range, xPos, yPos, width, height, title, xAxisTitle, yAxisTitle ) ;  
				 ws->Cells[ msgRow, msgCol ] = b ?  L"주파수(Y) 챠트 생성 완료." : L"주파수(Y) 챠트 생성 실패." ;  
			 } 

			 ws->Cells[ msgRow, msgCol ] = L"변환 작업이 완료되었습니다." ; 
			 
			 if( false == exApp->Visible ) {
				 exApp->Visible = true ; 
			 }
			 ...
			 if( false == exApp->Visible ) { 
				 // show message to save the excel file
				 System::String^ title = L"Success";
				 System::String^ msg = L"Excel file has been saved!.";

				 System::Windows::Forms::MessageBox::Show( msg, title, MessageBoxButtons::OK, MessageBoxIcon::Exclamation ) ;
			 }

			 // end of saving as excel file
		 }

private: bool AddChart(Excel::Worksheet^ ws, System::String^ range, int xPos, int yPos, int width, int height, System::String^ chartTitle, System::String^ xAxisTitle, System::String^ yAxisTitle ) {
			 ...
			 try {
				 ...
				 Excel::ChartObjects^ chObjs = safe_cast<Excel::ChartObjects^>(ws->ChartObjects(Type::Missing));

				 //6. Add the ChartObject to the collection at(x, y, width, height) in points
				 //   Width = 350 to prevent title from wrapping
				 Excel::ChartObject^  chObj = chObjs->Add(xPos, yPos, width, height );

				 //7. Create a chart from the ChartObject
				 Excel::Chart^ ch = chObj->Chart;

				 //8. Create a Range object & set the data range.
				 Excel::Range^ rn = ws->Range[ range, Type::Missing ];

				 //9. Do the chart
				 ch->ChartWizard(rn->CurrentRegion,        //Source
					 Excel::XlChartType::xlLine,    //Gallery
					 Type::Missing,          //Format
					 Excel::XlRowCol::xlColumns,    //Plot by
					 1,                      //Category Labels
					 1,                      //Series Labels
					 true,                   //Has Legend
					 chartTitle,                  //Title
					 xAxisTitle,                  //Category Title
					 yAxisTitle,                  //Value Title
					 Type::Missing);         //Extra Title

				 //10. Tell it the chart type again - initially comes up as a "lineMarked" type
				 ch->ChartType = safe_cast<Excel::XlChartType>(Excel::XlChartType::xlLine);

				 //11. Position the Chart Legend from the side to the bottom
				 ch->Legend->Position = Excel::XlLegendPosition::xlLegendPositionBottom;

				 //12. Format the Y-axis numbers to integers
				 //safe_cast<Excel::Axis^>(ch->Axes(Excel::XlAxisType::xlValue, Excel::XlAxisGroup::xlPrimary))->TickLabels->NumberFormat = "#,##0";

				 //13. Make the lines thick 
				 try {  
					 safe_cast<Excel::Series^>(ch->SeriesCollection(1))->Border->Weight = Excel::XlBorderWeight::xlThick;
					 //safe_cast<Excel::Series^>(ch->SeriesCollection(2))->Border->Weight = Excel::XlBorderWeight::xlThick;
				 } catch (System::Exception^  ) { 
				 }

				 //14. Change the line colors
				 try { 
					 safe_cast<Excel::Series^>(ch->SeriesCollection(1))->Border->ColorIndex = 3;
					 //safe_cast<Excel::Series^>(ch->SeriesCollection(2))->Border->ColorIndex = 32;
				 } catch( System::Exception^ ) { 
				 }

				 // end of adding a chart
			 } catch ( System::Exception^ ) { 
				 b = false ; 
			 }
